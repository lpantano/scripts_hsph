...
#render_2_drop("mrna-summary.Rmd", "jeffrey/hgmacro_model")
```
...
combs = list(c("control_none","KOsp140_none"), c("control_LPS","KOsp140_LPS"),
             c("control_none","control_LPS"), c("KOsp140_none","KOsp140_LPS"))
dds = DESeq(dds)
...
for (name in names(all_results)){
    ...
    deg_res[[name]] = degResults(res=res, dds, rlogMat=rlogMat, name, 
                    FDR=0.05, FC=0.05, group="treatment", xs="group",
                    org=org.Hs.eg.db, 
...
```{r fpkm}
fpkm = read.table(file.path(root_path,"combined.gene.sf.tpm"), header=T, row.names=1)
...
save_file(fpkm[peak0h_ens[!is.na(peak0h_ens)],], "combined_peaks_0h_tpm.csv", path_results)
save_file(fpkm[peak4h_ens[!is.na(peak4h_ens)],], "combined_peaks_4h_tpm.csv", path_results)
```
