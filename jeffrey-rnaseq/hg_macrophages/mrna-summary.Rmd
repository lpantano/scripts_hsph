...
for (name in names(res)[2:3]){
    ...
    stats = as.data.frame(df[,c(3,7)])
    text = read.csv(file.path(root_path, paste0(name, "_selected.csv")))
    text = df[as.character(text$id),c(3,7,8)]
    volcano_density_plot(stats, title=name, lfc.cutoff=log2(1.5), pval.cutoff = 0.05, plot_text = text)
}
...
out_sort = cbind(data.frame(ensembl=hr$labels[hr$order], 
                            ...
                            log2FC_LPS = -1 * lps[hr$labels[hr$order], "log2FoldChange"]), df[hr$labels[hr$order],])
save_file(out_sort, fn_out, path_results)
