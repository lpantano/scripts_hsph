...
library(DT)
basicConfig()
project_summary = "~/orch/scratch/henske_mirs/human/final/2016-02-12_human/project-summary.csv"
counts_file = "~/orch/scratch/henske_mirs/human/final/2016-02-12_human/combined.counts"
tx2genes_file = "~/orch/scratch/henske_mirs/human/final/2016-02-12_human/tx2gene.csv"
...
if (file.exists(tx2genes_file)) {
  ...
  sf_files = file.path("~/orch/scratch/henske_mirs/human/final/", rownames(summarydata), "sailfish",
...
# Expression ratio bweteen DK16 and DK19|DK22

Since we don't have enough replicates I can not give p-values. So I will
calculate the ration bewteen 19|22 and 16 samples. Expression values
are normalized and transformed to log2. 

```{r dk-ratio, cache=FALSE}
library(org.Hs.eg.db)
ma = data.frame(assay(vst))
pkp4 = rowMeans(ma[,c("DK19", "DK22")])
control = ma$DK16
ratio = cbind(data.frame(genes = as.character(rownames(ma)), 
                   log2FC = pkp4 - control,
                   stringsAsFactors = FALSE),
              data.frame(assay(vst)[,1:3])
              ) 
ratio$symbol = convertIDs(as.character(ratio$genes), "ENSEMBL",
                          "SYMBOL", org.Hs.eg.db, "useFirst")
ratio$desc = convertIDs(as.character(ratio$genes), "ENSEMBL",
                          "GENENAME", org.Hs.eg.db, "useFirst")
ratio = annotate_df(ratio, "genes",  'hsapiens_gene_ensembl', "ensembl_gene_id", "gene_biotype")
ratio = ratio[order(abs(ratio$log2FC), decreasing = TRUE),]


cat("\n\n")
sanitize_datatable(head(ratio, 200), 
                   style='bootstrap', 
                   filter="top") %>%
formatRound(2:5)
cat("\n\n")
...
fn = "pkp4_ratio.tsv"
save_file(ratio, fn, path_results)

```

In this table, only top 200 are shown,
to look at the whole table open **`r fn`**

# Expression ratio bweteen HL samples


```{r hl-ratio, cache=FALSE, results='asis'}
cmb = list(c("HL11", "HL10"),
           c("HL12", "HL10"),
           c("HL12", "HL11"))
.void = lapply(cmb, function(names){
  s1 = ma[, names[1] ]
  s2 = ma[, names[2] ]
  ratio = cbind(data.frame(genes = as.character(rownames(ma)), 
                           log2FC = s1 - s2,
                           stringsAsFactors = FALSE),
                data.frame(assay(vst)[, names])
  ) 
  ...
  fn = paste0(names[1], "_vs_", names[2], "_ratio.tsv")
  save_file(ratio, fn, path_results)
  cat("\nFull table at ", fn, "\n")
  head(ratio, 200)
})
...
name_res = compress_results(path_results, "human_pilot")
```
