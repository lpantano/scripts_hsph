...
## Difference miRNA targets

```{r mirna-diff-targets,results='asis'}
knitr::kable(merge(n2_both_res_sign %>% group_by(mir) %>% dplyr::summarise( n2 = n()) ,
...
same_gene = inner_join(n2[,1:2],ko[,1:2],by="g") %>% filter(! mir.x %in% mir.y)
