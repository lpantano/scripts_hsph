...
dd$sample = c(rep("s1",4), rep("s2",4), rep("s3",4) , rep("s4",4),
              rep("s5",4) , rep("s6",4))
ggplot( dd, aes( x=sample, y=V2, fill=V1 ) ) +
    geom_bar( stat = 'identity', position = 'dodge') + 
    scale_fill_brewer( palette = "Set1" ) +
...
We detect the gene ZCCHC6 as down-regulated in the knowcdown samples.
...
total_exp["ENSG00000083223",]
```

In general, it seems there is an up-regulation of polyA genes, and down-regulation 
of polyA-U.. genes. But counts are quite small, so there is not enough power to detect
statistically this trend. 
...
lapply(names(deg_list),plot_DEG)
```
