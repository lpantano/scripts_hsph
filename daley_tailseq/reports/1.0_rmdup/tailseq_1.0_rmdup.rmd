...
dn <- "daley_tailseq/1.0_rmdup"
root_path<-"~/ody/projects/daley_mRNA_mod/tailseq-112014"
path_files = "~/repos/pipelines/daley_tailseq/reports/1.0_rmdup/"
```

```{r render,eval=FALSE}
knitr::opts_chunk$set(tidy=TRUE, highlight=TRUE, dev="png",
                      cache=TRUE, highlight=TRUE, autodep=TRUE, warning=FALSE, error=FALSE,
                      cache.path="~/.cache/tailseq-report/html_rmdup_cached/", eval=TRUE,
                      message=FALSE, prompt=TRUE, comment='', fig.cap='', bootstrap.show.code=FALSE)
render_2_drop("~/repos/pipelines/daley_tailseq/reports/1.0_rmdup/tailseq_1.0_rmdup.rmd",dn)
```


```{r load,cache=FALSE}
setwd(root_path)
s1_raw = read.table(paste0(root_path,"/work_rmdup/s1_summary.dat"))
s2_raw = read.table(paste0(root_path,"/work_rmdup/s2_summary.dat"))
s3_raw = read.table(paste0(root_path,"/work_rmdup/s3_summary.dat"))
s4_raw = read.table(paste0(root_path,"/work_rmdup/s4_summary.dat"))
```
...
all = lapply(names(dd_list) , function (name){
        ...
        x=dd_list[[name]]    
        get_size_gene(x,limit=10) %>% mutate(sample=name)
        })
...
all = lapply(names(dd_list) , function (name){
        #x = normalization(dd_list[[name]])
        x=dd_list[[name]]
        rbind(get_size_gene(x,"<15",5) %>% mutate(sample=name),
        get_size_gene(x,"<25",5) %>% mutate(sample=name),
        get_size_gene(x,">25",5) %>% mutate(sample=name) )
