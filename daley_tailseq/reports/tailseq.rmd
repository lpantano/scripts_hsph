...
```{r polya-filter}
s1 = read.table(paste0(root_path,"/work/filter_polya6/s1_summary.dat"))
s1 = filter_out(s1,100)
s2 = read.table(paste0(root_path,"/work/filter_polya6/s2_summary.dat"))
s2 = filter_out(s2,100)
s3 = read.table(paste0(root_path,"/work/filter_polya6/s3_summary.dat"))
s3 = filter_out(s3,100)
s4 = read.table(paste0(root_path,"/work/filter_polya6/s4_summary.dat"))
s4 = filter_out(s4,100)
...
sp_rw_f = sp_reworked %>% group_by(V1) %>% summarise(polyA=sum(V2)) %>% ungroup()


sp_rw_genes = subset(sp_rw_f,polyA>=30)[,1]
venn(list(public=public,rw=sp_rw_genes))  
```
...
all_genes = rbind(data.frame(gene=genes_s1[[1]],sample="s1",is=1),
      ...
      data.frame(gene=rm_na(sp.f[sp.f[,3]>1,1]),sample="public",is=1)
      )
...
detect = rowSums(all_genes[,2:6])
all_genes_sum = all_genes %>% group_by(is.s1, is.s2, is.s3, is.s4,is.public) %>% summarise(total = n()) %>% ungroup()
per = apply( all_genes_sum, 1, function(x){
     unlist((x[6]/as.vector(total_genes[,2])) * x[1:5] *100 )
    })
per[per==0] = NA
all_genes_sum = cbind( all_genes_sum, t(per) )
#cols = brewer.pal(5,"")
aheatmap( t(as.matrix(all_genes_sum[,7:11])), annCol = all_genes_sum[,1:5], Colv = NA, Rowv = NA,cellheight = 45,  fontsize=9)
```
