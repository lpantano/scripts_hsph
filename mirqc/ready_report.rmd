...
### abundance detection miRQC samples
...
the logFC is 0.5 that is a FC of log2(1/0.75)=0.5 that should be C according to description but is D
...
the logFC is 1.6 that is a FC of log2(1/0.25)=2 that should be D according to description but is C
...
what is the same that is in the GEO data set description.
...
According to the text they saw cross-mapping between this miRNAs, but here we are seeing perfect annotation.
I think the method was causing this, since I am not seeing this with seqbuster.
...
As an example of some figures you can do with the package. There is one figure per type of
isomiR (read more [here](http://lpantano.github.io/isomiRs)). The yaxes shows the percentage of unique sequences with that change.
The xaxes shows the percentage of abundance with that change.
...
Seems there are some errors for serum and MS2 samples at position 13/14, 
...
The same logic is applied to clusters detection.
...
clusters which mirqca > mirqcd > mrqcc are `r sum(conc_a)` out of `r sum(top_a)`
clusters which mirqcb > mirqcc > mrqcd are `r sum(conc_b)` out of `r sum(top_b)`
...
The exactly same thing than we saw for miRNA analysis.
