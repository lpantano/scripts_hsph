...
plotPCA(vsd_local, intgroup=c("condition"), ntop=100) + geom_text(aes(label=summarydata$Name[1:6]),hjust = 0, nudge_x = 0.3)
...
idx = c(1,2,4:6)
counts_filter <- counts[rowSums(counts>0)>2, idx]
dds_filter = DESeqDataSetFromMatrix(countData=counts_filter,
    colData=summarydata[idx,], design = ~ condition)
dds_filter = DESeq(dds_filter)

vsd_local <- varianceStabilizingTransformation(dds_filter)

plotPCA(vsd_local, intgroup=c("condition"), ntop=100) + geom_text(aes(label=summarydata$Name[idx]),hjust = 0, nudge_x = 0.3)

all_results_filter = handle_deseq2(dds_filter, summarydata[idx,], "condition",list(c("plusP4", "minusP4")))
