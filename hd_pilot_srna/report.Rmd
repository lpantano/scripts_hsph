...
root_path = "~/orch/scratch/simulator/hd/srna/final"
root_file = file.path(root_path, "srna_out_files")
dir.create(root_file, showWarnings = FALSE)

fns = list.files(root_path, recursive=T, full.names = TRUE)

metadata_fn =  list.files(file.path(root_path,"2016-11-20_srna","report"), pattern = "summary.csv$",recursive = T, full.names = T)
metadata = read.csv(metadata_fn, row.names=1)
...
files = fns[grepl("trimming_stats", fns)]
...
```{r mirna-load}
files = fns[grepl("mirbase-ready", fns)]
ismirbase = length(files) > 0
mirdeep2_files = fns[grepl("novel-ready", fns)]
...
counts = counts(obj)
dds = DESeqDataSetFromMatrix(counts[rowSums(counts>0)>0,], colData=design, design=~1)
vst = rlog(dds)

pheatmap(assay(vst), annotation_col = design[,1,drop=FALSE], show_rownames = F, 
...
fn_json = list.files(file.path(root_path, "2016-11-20_srna", "seqcluster"),pattern = "seqcluster.json",recursive = T,full.names = T)
