...
# pheatmap(raw[positive,], annotation = meta, clustering_distance_col="correlation", clustering_method = "ward.D2")
# pheatmap(raw[negative,], annotation = meta, cluster_rows = FALSE, cluster_cols = FALSE)
# pheatmap(raw[hk,], annotation = meta, clustering_distance_col="correlation", clustering_method = "ward.D2")
...
dse = DESeqDataSetFromMatrix(raw_f, meta,~ Run + Treatment)
...
for (i in names(res)){
  ...
  write.table(as.data.frame(res[[i]]),paste0(i,"_mirna_de.tsv"),sep="\t",quote=F)
  cat("\n\n") 
}
