...
find_protein = function(mirs, mrna, mirna, mirdb_red){
    common = intersect(as.character(colnames(mrna)), as.character(rownames(dd)))
    res = data.frame()
    for (mir in mirs){
        ...
        if (nrow(sign)>0){
            targets = mirdb_red %>% 
                filter(precursor %in% c(gsub("[::.::]","-",gsub("mir","miR",mir)),"hsa-miR-92a-1","hsa-miR-92a")) %>% 
...
mat = read.table("~/orch/scratch/gregory_pca/extracted_data/mir-hg18-lusc.counts.txt", header=T, check.names=F, row.names=1)
colnames(mat) = sapply(colnames(mat),clean_id)

meta = read.table("~/orch/scratch/gregory_pca/extracted_data/mrna-lusc.metadata.txt", header=T, check.names=F, row.names=1)
...
voom_m = voom(mat[rowSums(mat[,common])>1,common])
...
dd_high = dd %>% filter(isy1>1000)

ggplot(dd_high) +
    geom_point(aes(y=hsa.mir.18a-hsa.mir.92a.1,x=log2(isy1))) +
    facet_wrap(~class, nrow=2)


r = dd_high$hsa.mir.18a - dd_high$hsa.mir.92a.1
cor.test(r, log2(dd_high$isy1))

ggplot(dd_high) +
    geom_point(aes(y=hsa.mir.17-hsa.mir.92a.1,x=log2(isy1))) +
    facet_wrap(~class, nrow=2)

r = dd_high$hsa.mir.17 - dd_high$hsa.mir.92a.1
cor.test(r, log2(dd_high$isy1))
...
r = 2^(dd_high$hsa.mir.18a)/2^(dd_high$hsa.mir.92a.1)
cor.test(r, dd_high$isy1)
```
...
## protein-ratio correlation analysis

We took any protein that correlated with the ratio (mirX/mir-92a-1) FDR < 10%
...
hist(abs(res_lusc$cor), main="Distribution of correlation values")
write.table(res_lusc, "lusc_corr_values.tsv", row.names=F, quote=F, sep="\t")
```

[download lusc correlation table](`r get_report_links("lusc_corr_values.tsv")`)
...
dd[common, "isy1"] = unlist(mrna[isy1,common])
d_high = dd %>% filter(isy1>800)

ggplot(d_high) +
    geom_point(aes(y=hsa.mir.17-hsa.mir.92a.1,x=log2(isy1),colour="18-isy1")) +
    facet_wrap(~class, nrow=2)

r = d_high$hsa.mir.17 - d_high$hsa.mir.92a.1
cor.test(r, log2(d_high$isy1))
```
...
hist(abs(res_coad$cor), main="Distribution of correlation values")
write.table(res_coad, "coad_corr_values.tsv", row.names=F, quote=F, sep="\t")
```

[download coad correlation table](`r get_report_links("coad_corr_values.tsv")`)
