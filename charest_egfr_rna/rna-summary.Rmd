...
```{r qc-setup}
project_summary = "~/orch/scratch/charest_egfr_srna/RNA/egfr_rna/final/2016-09-29_egfr_rna/project-summary.csv"
counts_file = "~/orch/scratch/charest_egfr_srna/RNA/egfr_rna/final/2016-09-29_egfr_rna/combined.counts"
tx2genes_file = "~/orch/scratch/charest_egfr_srna/RNA/egfr_rna/final/2016-09-29_egfr_rna/tx2gene.csv"
...
pca_loadings = function(object, ntop=500) {
  ...
  return(pca)}
pc = pca_loadings(normalized_counts)
...
pca_plot = function(comps, nc1, nc2, colorby) {
   c1str = paste0("PC", nc1)
   c2str = paste0("PC", nc2)
  ggplot(comps, aes_string(c1str, c2str, color="size", shape="group")) +
    geom_point() + theme_bw() +
...
```{r spikein}
library(readr)
spike = read_tsv(file.path(dirname(project_summary), "spikein.sf"))
ggplot(spike, aes(x=sample, y=numreads)) + geom_jitter() + scale_y_log10()
```
...
mac_rank = do.call(cbind, lapply(colnames(counts), function(col){
    idx = sort(counts[,col], decreasing = T,index.return=T)
    r <- 1:length(idx$ix)
    r[order(idx$ix)]
}))
row.names(mac_rank) = row.names(counts)
colnames(mac_rank) = colnames(counts)

root_file = file.path(dirname(project_summary), "rna_out_files")
dir.create(root_file, showWarnings = FALSE)

remove = c("ENSMUSG00000106106", "ENSMUSG00000105361")
y = DGEList(counts=counts[!(row.names(counts) %in% remove),])
y = calcNormFactors(y,method = "upperquartile")

counts$genename = convertIDs(row.names(counts), "GENEID", "GENENAME", EnsDb.Mmusculus.v79, ifMultiple = "useFirst")
save_file(counts, "rna_counts.csv", root_file)
normalized_counts = as.data.frame((normalized_counts))
normalized_counts$genename = convertIDs(row.names(normalized_counts), "GENEID", "GENENAME", EnsDb.Mmusculus.v79, ifMultiple = "useFirst")
save_file(normalized_counts, "rna_norm_counts.csv", root_file)

normalized_counts = cpm(y, normalized.lib.sizes=TRUE, log = TRUE)
normalized_counts = as.data.frame((normalized_counts))
normalized_counts$genename = convertIDs(row.names(normalized_counts), "GENEID", "GENENAME", EnsDb.Mmusculus.v79, ifMultiple = "useFirst")
save_file(normalized_counts, "rna_norm_remove_rRNA_counts.csv", root_file)

mac_rank = as.data.frame(mac_rank)
mac_rank$genename = convertIDs(row.names(mac_rank), "GENEID", "GENENAME", EnsDb.Mmusculus.v79, ifMultiple = "useFirst")
save_file(mac_rank, "rna_rank_counts.csv", root_file)
