...
## Correlation of raw counts with size

```{r cor-size}
ref_fn = paste0(root_path, "/ref-transcripts.db")
ref=read.table(ref_fn,sep="\t") %>% filter(grepl("transcript",V3)) %>% mutate(size=V5-V4+1) %>% group_by(V10) %>% summarise(average=mean(size))
dd = as.data.frame(counts)
dd$gene = row.names(counts)
dd$size = match(ref$V10,dd$gene)
dd = melt(dd,id.vars = c("gene","size")) %>% filter(size<1000) %>% mutate(size_tag=cut(size,breaks=c(0,50,100,2000)), exp=cut(value,breaks=c(-1,0,1e23),labels=c("No","Yes")))
...
```{r heatmap-raw}
pheatmap(cor(counts),annotation = summarydata[,c("group","batch")])
```
...
pheatmap(cor(normalized_counts),annotation = summarydata[,c("group","batch")])
```

## Correlation (Spearman) heatmap of TMM-normalized counts
```{r spearman-heatmap-normalized}
pheatmap(cor(normalized_counts, method="spearman"),annotation = summarydata[,c("group","batch")])
```
...
```{r rlog-heatmap}
pheatmap(cor(rlogMat, method="spearman"),annotation = summarydata[,c("group","batch")])
```
...
knitr::kable(tab_anno %>% filter(grepl("TRMT",external_gene_name)))
```
...
tab_anno %>% filter(chromosome_name=="MT")
```
...
knitr::kable(tab_anno_non_filtered %>% filter(grepl("TRMT",external_gene_name) & pvalue<0.05 ) )
knitr::kable( tab_anno_non_filtered %>% filter( grepl("MT-T",external_gene_name) & pvalue<0.06 ) )
...
pheatmap(ma, clustering_distance_rows = "correlation", clustering_distance_cols = "correlation", clustering_method = "ward",scale = "row")
...
mcols(mit_gr)$pvalue = tab_anno_single[ unlist(mcols(mit_gr)$gene_id) , "pvalue" ]
mcols(mit_gr)$padj = tab_anno_single[ unlist(mcols(mit_gr)$gene_id) , "padj" ]
...
```{r}
knitr::kable(tab_anno_single %>% filter(grepl("TRMT",external_gene_name) & pvalue<0.05 ) )
knitr::kable( tab_anno_single %>% filter( grepl("MT-T",external_gene_name) ) )
```
...
```{r}
knitr::kable(tab_anno_all %>% filter(grepl("TRMT",external_gene_name) & pvalue<0.05 ) )
knitr::kable( tab_anno_all %>% filter( grepl("MT-T",external_gene_name) ) )
```
...
tracks(genes,gplot_cov,xlim = c(3932,4744 ) )


tracks(genes,gplot_cov+ylim(0,2000),xlim = c(4250,4470 ))
...
tracks(genes,gplot_cov,xlim = c(8230,8450 ) )


tracks(genes,gplot_cov+ylim(0,2000),xlim = c(8290,8369 ))
