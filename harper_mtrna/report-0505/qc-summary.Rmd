...
dn <- "harper_mtrna_0505/"
root_path<-"~/orch/scratch/harper_mitrna_2/data_May_01/mitrna/final/"
merged_path<-"~/orch/scratch/harper_mitrna_2/data_May_01/mitrna/final/merge/"
path_files = "~/repos/pipelines/harper_mtrna/report-0505/"
...
library(pheatmap)
library(dplyr)
project_summary = "~/orch/scratch/harper_mitrna_2/data_May_01/mitrna/final/2015-05-04_mitrna/project-summary.csv"
counts_file = "~/orch/scratch/harper_mitrna_2/data_May_01/mitrna/final/2015-05-04_mitrna/combined.counts"
...
melted$sample = factor(melted, levels=levels(summarydata$Name))
...
keep = as.character(summarydata$Name[!grepl("t2-t1", summarydata$Name)])
...
read_cov = function(in_file){
    samples = c("c1-ready","c2-ready","c3-ready",
            "t1-t1-ready","t3-t1-ready",
            "t1-t2-ready","t2-t2-ready", "t3-t2-ready")
...
for ( gene_name in regions$V6 ) {
    ...
    min_pos = coor$V4-100
    max_pos = coor$V5+100
    max_trn = mean(sort(dd[dd$V2>coor$V4+5 & dd$V2<coor$V5-5,"norm"], decreasing=TRUE)[1:10]) + 20

    gplot_cov = ggplot( dd, aes(y=norm,x=V2, colour=sample) ) +
        geom_line() +
        scale_color_manual(values = c(colfuncGreen(3),colfuncRed(3),colfuncBlue(2))) 
...
fpkm = read.table("~/orch/scratch/harper_mitrna_2/data_May_01/mitrna/final/2015-05-04_mitrna/combined.fpkm",row.names=1, header=T)
