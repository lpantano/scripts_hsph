...
# root_path = "~/orch/scratch/vishal_mirna_kidney/UUO-model/mrna/final/2016-02-03_mrna/"
path_results = file.path(root_path, "files_publish")
...
res =  results(dds)
deg_res = degResults(res, dds, rlogMat, "uuo_model", 
                    FDR=0.01, FC=4, group=NULL, xs="group",
                    org=org.Mm.eg.db, 
...
## Clustering in common patterns {.tabsets}
...
```{r clustering-res, results='asis'}
clusters = degPatterns(rlogMat[deg_res$sign,], metadata, minc = 15, summarize = "group", time="group", col = NULL)
.df=clusters$df
.df$symbol = convertIDs(rownames(.df), "ENSEMBL", "SYMBOL", org.Mm.eg.db, "useFirst")
save_file(.df, "clusters_genes.tsv", path_results)
```

```{r clustering-fig, results='asis', fig.width=12, fig.height=12}
