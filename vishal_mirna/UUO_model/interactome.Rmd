...
# render_2_drop("interactome.Rmd", "vishal_kidney/UUO_small_rna_newcode")
```
...
```{r load-files}
ma_rda = "~/orch/scratch/vishal_mirna_kidney/UUO-model/mirna_bcbio/final/files/ma.rda"
mirna_results = "~/orch/scratch/vishal_mirna_kidney/UUO-model/mirna_bcbio/final/files/uuo_model_mirna_log2_counts.tsv"
mirna_design = "~/orch/scratch/vishal_mirna_kidney/UUO-model/mirna_bcbio/final/2016-04-21_mirna_bcbio/report/summary.csv"
mirna_de = "~/orch/scratch/vishal_mirna_kidney/UUO-model/mirna_bcbio/final/files/uuo_model_mirna.tsv"

mrna_results = "~/orch/scratch/vishal_mirna_kidney/UUO-model/mrna/final/2016-02-03_mrna/files/uuo_model_log2_counts.tsv"
mrna_design = "~/orch/scratch/vishal_mirna_kidney/UUO-model/mrna/final/2016-02-03_mrna/project-summary.csv"
mrna_de = "~/orch/scratch/vishal_mirna_kidney/UUO-model/mrna/final/2016-02-03_mrna/files/uuo_model.tsv"

mirna_ma = as.matrix(read.table(mirna_results, header=T, row.names=1,sep=","))
mrna_ma = as.matrix(read.table(mrna_results, header=T, row.names=1,sep=","))
...
write.table(data.frame(m=mirna_keep), "~/orch/scratch/vishal_mirna_kidney/UUO-model/mirna_bcbio/final/files/mirna_diff.txt",row.names=F, col.names=F, quote=F)
...
# run seqcluster at this point to create matrix.tsv
# /groups/bcbio/bcbio/anaconda/bin/seqcluster target --input mirna_diff.txt --sps mmu -o target --annotation /groups/bcbio/bcbio/genomes/Mmusculus/mm10/srnaseq

if ( !file.exists(ma_rda) ){
    pairs=read.table("~/orch/scratch/vishal_mirna_kidney/UUO-model/mirna_bcbio/final/files/target/matrix.tsv") %>% filter(V3< -.1)
...
name_res = compress_results(root_file, prefix = "uuo_model_mirna", zip = TRUE)
```
