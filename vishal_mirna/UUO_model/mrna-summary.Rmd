...
path_results = "~/orch/scratch/vishal_mirna_kidney/UUO-model/mrna/final/2016-02-03_mrna/files"
dir.create(path_results, showWarnings = FALSE)
```

```{r create-report, echo=FALSE, eval=FALSE}
#render_2_drop("mrna-summary.Rmd", "vishal_kidney/UUO_mrna")
```
...
basicConfig()
project_summary = "/home/lpantano/orch/scratch/vishal_mirna_kidney/UUO-model/mrna/final/2016-02-03_mrna/project-summary.csv"
counts_file = "/home/lpantano/orch/scratch/vishal_mirna_kidney/UUO-model/mrna/final/2016-02-03_mrna/combined.counts"
tx2genes_file = "/home/lpantano/orch/scratch/vishal_mirna_kidney/UUO-model/mrna/final/2016-02-03_mrna/tx2gene.csv"
...
if (file.exists(tx2genes_file)) {
  ...
  sf_files = file.path("/home/lpantano/orch/scratch/vishal_mirna_kidney/UUO-model/mrna/final/",
...
metadata$group = factor(metadata$group, levels = c("normal", "day1",
                                                   "day2", "day3",
                                                   "day7", "day14"))
```
...
all_results = list()
all_results[["uuo_model"]] = results(dds)
...
#   print(p)
  ...
  cat("\n\n### Heatmap most significand, padj<0.05 and log2FC > 4\n")
  sign = row.names(out_df)[out_df$padj<0.01 & !is.na(out_df$padj) & abs(out_df$log2FoldChange) > 4]
  ...
  .idx = .res$padj<0.01 & abs(.res$log2FoldChange) > 4
  .idx[is.na(.idx)] = FALSE
...
norm_sign = t(apply(rlogMat[sign,], 1, function(e){
  m = sapply(metadata$group, function(i){
    idx = which(metadata$group == i)
...
plots = lapply(to_plot, function(x){
  plot_cluster(norm_sign, as.character(names(groups[groups==x])), metadata, x)
  })
...
save_file(data.frame(do.call(rbind, .void)), "clusters_genes.tsv", path_results)
...
name_res = compress_results(path_results,prefix = "uuo_results")
```
