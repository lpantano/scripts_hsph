...
# render_2_drop("omics.Rmd", "vishal_kidney/UUO_omics")
```
...
result_files = "~/orch/scratch/vishal_mirna_kidney/UUO-model/omics/files"
...
protein_file = file.path(root_path, "protein", "files")
dir.create(protein_file, recursive = T, showWarnings = F)
counts = read.csv(file.path(root_path, "UUO_Proteomics.csv"))
row.names(counts) = counts$Uniprot.ID
clean_counts = counts[,8:17]

mapping = data.frame(id=rownames(clean_counts))
...
idx = match(mapping2$id,counts$Gene.Symbol)
...
clean_counts = clean_counts[mapping$id,]
rownames(clean_counts) = mapping$ensembl_gene_id
cat("## Expression density of 'raw' data\n\n")
ggplot(melt(clean_counts), aes(x=value, colour=variable)) + geom_density()

MA <- normalizeBetweenArrays(as.matrix(clean_counts), method="quantile")
...
absMaxFC = rowMeans(abs(res[,1:4]))
sign = row.names(res[absMaxFC>2 & res$adj.P.Val<0.01,])
# protein_sign = row.names(res[absMaxFC>2 & res$adj.P.Val<0.01,])
...
name_res = compress_results(protein_file, prefix = "uoo_model_prot", zip = TRUE)
...
root_path = "~/orch/scratch/vishal_mirna_kidney/UUO-model"
mrna_path = "mrna/final/2016-02-03_mrna/files"
...
mirna_path = "mirna_bcbio/final/files"
...
# length(intersect(rownames(prot_matrix), rownames(mrna_matrix)))
```
...
merged_all = merge(mirna_merged, prot_merged, by=1, all=TRUE)
...
name_res = compress_results(result_files, prefix = "uuo_model_omic", zip = TRUE)
```
