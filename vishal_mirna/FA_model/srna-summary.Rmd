...
root_path = "/home/lpantano/orch/scratch/vishal_mirna_kidney/FA_model/mirna_bio/final"
...
obj_mirdeep <- IsomirDataSeqFromFiles(files = files[ronames(design)], design = design , header = T)
```
...
print_out = function(dss, rlog=NULL, res=NULL, prefix="data_"){
  ...
  if (length(sign)<2){
    ...
    print(mds(rlogmat[sign,], condition = colData(dss)$condition))
  ...
  plots = lapply(to_plot, function(x){
    plot_cluster(norm_sign, as.character(names(groups[groups==x])), design, x)
  })
...
```{r de}
counts = counts(obj)
design$condition = factor(design$condition, levels = c("Day0", "Day1",
                                                   "Day2", "Day3",
                                                   "Day7", "Day14",
                                                   "Day28"))
...
print_out(dss, prefix="mirna_")

```


```{r DESeq-confirmation, eval=FALSE}
DESeq2::plotCounts(dss, "mmu-let-7a-5p", "condition" )
...
print_out(dss_mirdeep2, prefix="novel_")
...
print_out(dss_iso, prefix="isomirna_")
...
print_out(dss_clus, prefix="clus_")
