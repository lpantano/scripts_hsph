...
The bad thing is there is no 22 nt long enrichmend, so no miRNA enrichment.
...
* red : sequences aligned
* blue : sequences that are part of clusters of sequences (not sequences that are
alone in one region)
* green: sequences inside miRNAs
* purple: other types like genes, snoRNAs
* orange: sequences in tRNAs
...
The bigger fragments(sequences without adapter) don't seem to be transcriptomic. There is big ammount that are unkown.
...
## detection of unknown fragments
I will map the a fraction of the sequences to the patents sequences. Normally there are primers of different protocols

```{r pat}
path_pat = paste0( root_path, "/work/exosomes_large/work/trimmed" )
tab = data.frame()
for (file in list.files(path_pat, ".counts")){
    content = read.table( paste0(path_pat, "/", file) ) 
    top = content[ order(content$V1, decreasing = T), ]
    tab = rbind(tab, data.frame( sample=file, seq = top[1,2], abundance = top[1,1]/1000 * 100 ))
}
knitr::kable(tab)
```

There are a big ammount of other stuff there. Look for the meaning in this page:

http://www.ncbi.nlm.nih.gov/nuccore/GI_ID (GI_ID is the number after "gi|" 
in the `seq` column).

("http://www.ncbi.nlm.nih.gov/nuccore/550574664")
