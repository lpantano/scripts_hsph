...
dn <- "yellen_scneuron/report_redone"
files_path = "~/repos/pipelines/yellen_scneuron/report_redone/"
...
render_2_drop("~/repos/pipelines/yellen_scneuron/report_redone/qc-summary.Rmd",dn)
```
...
library(CHBUtils)
library(pheatmap)
project_summary = "~/orch/scratch/yellen_singlecell_neuron/redone/scneuron/work/project-summary.csv"
counts_file = "~/orch/scratch/yellen_singlecell_neuron/redone/scneuron/work/htseq-count/combined.counts"
...
ggplot(melted %>% filter(biotype=="protein_coding" & sample=="SSF339-C2_R"), aes(x=count,y=gene)) +
    geom_point()  +
...
```{r mds-normalized}
mds(normalized_counts, d = "cor", k=length(colnames(normalized_counts)) - 1, condition = summarydata$rRNA.rate)
mds(normalized_counts, d = "cor", k=length(colnames(normalized_counts)) - 1, condition = summarydata$Genes.Detected)
```
...
[get ionchannel table](`r get_report_links(fn_ichannel)`)
[get total table](`r get_report_links(fn_ann)`)
