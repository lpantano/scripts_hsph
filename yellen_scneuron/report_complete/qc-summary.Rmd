...
melted$sample = reorder(melted$sample, colnames(counts))
#melted$count = log(melted$count)
ggplot(melted, aes(x=sample, y=count)) + geom_violin() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) + xlab("") +
    theme_bw(base_size=12) +
    theme(panel.grid.major = element_line(size = .5, color = "grey"))
```
...
all_ma = ensembl2name_ma(row.names(wed))
all_ma = all_ma[!is.na(all_ma$ENSEMBL),]
idx = match(all_ma$ENSEMBL,rownames(wed))
ann_wed = cbind(all_ma,wed[idx,])
fn_ann = paste0(files_path, "ann_log2_norm_counts.txt")
write.table(ann_wed,fn_ann,row.names=F,quote=F,sep="\t")
