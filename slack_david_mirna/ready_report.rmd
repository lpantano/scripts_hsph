...
root_path = "~/orch/scratch/david_slack_miRNA/work"
root_file = paste0("files/")
...
render_2_drop("ready_report.rmd", "david_slack_mirna")
```
...
do_de = function(raw, summarydata, condition, minc=3){
    dss = DESeqDataSetFromMatrix(countData = raw[rowMeans(raw)>minc,],
                       ...
                       design = ~ condition)
    dss = DESeq(dss)
    DESeq2::plotDispEsts(dss)
    dss
}

do_norm = function(dss, res, root_file, prefix){
    ...
    for (name in names(res)){
      fn_res = paste0(root_file, prefix, name, "_de.txt")
      write.table(res[[name]], fn_res,sep="\t")
    }
}
...
```{r DESeq-confirmation}
DESeq2::plotCounts(dds, "cel-miR-794-5p")
DESeq2::plotCounts(dds, "cel-miR-237-5p")

```

```{r tables}
do_norm(dds, all_results, root_file, "mirna_")
```
...
```{r DESeq-confirmation-iso}
DESeq2::plotCounts(dds, "cel-miR-1829c-5p.iso.t5:0.t3:u-A.ad:0.mm:5TG")
DESeq2::plotCounts(dds, "cel-miR-237-5p.iso.t5:0.t3:d-T.ad:0.mm:0")

```

```{r tables-iso}
do_norm(dds, all_results, root_file, "isomirs_")
```
...
```{r tables-c}
do_norm(dds, all_results, root_file, "clusters_")
```
...
```{r load-novel}
fn = list.files(file.path(root_path, "mirdeep2", "novel_res"), pattern = "clean", full.names = TRUE)
obj = IsomirDataSeqFromFiles(fn, design)
```
...
```{r DESeq-output-novel, results='asis'}
all_results = handle_deseq2(dds, design, condition)
plot_MA(all_results)
...
```{r DESeq-confirmation-novel}
DESeq2::plotCounts(dds, "cel-IV_35256-5p")
DESeq2::plotCounts(dds, "cel-II_11977-3p")
DESeq2::plotCounts(dds, "cel-IV_39449-5p")

```

```{r tables-novel}
do_norm(dds, all_results, root_file, "novel_")
```

# Files

```{r files, results='asis'}
for (f in list.files("files", full.names = T)){
  cat("[",basename(f),"]","(",get_report_links(f),")\n\n", sep = "")
}
