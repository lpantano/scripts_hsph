...
plot_gene = function(name, d){
  design = colData(d)
  ggplot(plotCounts(d, name, returnData=TRUE) %>% mutate(gender=design$gender, age=as.factor(design$age)), aes(x=condition,y=count,color=age,shape=gender)) + 
    geom_point(size=3) +
    ggtitle(name) + 
...
designma = metadata %>% mutate (age=round(AGE/10)*10, gender=GENDER, condition=X.1, diagnosis=SAMPLEDIAGNOSIS ) %>% dplyr::select(diagnosis, age, gender, condition)
...
ma = do.call(cbind, .list)
designma = metadata %>% mutate (age=round(AGE/10)*10, gender=GENDER, condition=X.1) %>% dplyr::select(age, gender, condition)
rownames(designma) = colnames(ma)
...
```{r DESeq-iso-output, results='asis', fig.height=9}
all_results = handle_deseq2(dds, designma, "condition")
plot_MA(all_results)
