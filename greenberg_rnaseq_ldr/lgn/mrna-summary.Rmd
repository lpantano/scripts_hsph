...
#render_2_drop("mrna-summary.Rmd", "greenberg/lgn_model")
```
...
combs = list(c("0", "1"), c("1", "3"), c("1", "8"), c("3", "8"), c("0", "3"), c("0", "8"))
dds = DESeq(dds)
all_results = handle_deseq2(dds, colData(dds), "group", combs)
deg_res = list()
for (name in names(all_results)){
    res = all_results[[name]]
    deg_res[[name]] = degResults(res=res, dds, rlogMat=rlogMat, name, 
                    FDR=0.05, FC=0.05, group=NULL, xs="group",
                    org=org.Mm.eg.db, 
...
for ( i in 1:length(deg_res)){
    keep = deg_res[[i]]$table$padj<0.1 & !is.na(deg_res[[i]]$table$padj)
    all = c(all,row.names(deg_res[[i]]$table)[keep])
}
```
...
name_res = compress_results(path_results,prefix = "lgn_results")
```
